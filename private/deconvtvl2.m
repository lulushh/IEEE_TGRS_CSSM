...
%     opts.gamma     - update constant for rho_r {2}rho_o
...
for itr=1:max_itr
    ...
    W2 = sum(W1,2);
    W2 = 1./(1+5.*W2);
    W_v = zeros(rows,cols,frames);
    for i = 1:frames
        W(i) = W2(:,:,i)./ rows*cols*frames;
        Df1_1(:,:,i) = Df1(:,:,i).*W(i);
        Df2_1(:,:,i) = Df2(:,:,i).*W(i);
        Df3_1(:,:,i) = Df3(:,:,i).*W(i);
        %W_v(((i-1)*rows*cols+1):(i*rows*cols))=W(i);
        W_v(:,:,i) = W(i);
    end 
